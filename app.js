...
'Мы создали дружелюбную атмосферу для маленьких пациентов. Наши врачи умеют находить подход к каждому ребёнку, превращая визит в увлекательное приключение. Лечим молочные и постоянные зубы бережно и без страха.':'Біз кішкентай пациенттер үшін достық атмосфера жасадық. Біздің дәрігерлер әрбір балаға тәсіл тауып, стоматологқа баруды қызықты шытырман оқиғаға айналдырады. Сүт және тұрақты тістерді ұқыпты және қорқынышсыз емдейміз.',
'Восстановление утраченных зубов с помощью дентальных имплантов — надёжный и долговечный метод. Мы используем импланты ведущих мировых производителей и проводим операции с минимальной травматичностью.':'Дентальды имплантаттардың көмегімен жоғалған тістерді қалпына келтіру — сенімді және ұзақ мерзімді әдіс. Біз әлемнің жетекші өндірушілерінің имплантаттарын қолданамыз және минималды жарақатпен операциялар жүргіземіз.',
'Красивая улыбка и правильный прикус для детей и взрослых. Мы подбираем оптимальный вариант лечения: от классических брекетов до невидимых элайнеров. Составляем индивидуальный план для каждого пациента.':'Балалар мен ересектерге арналған әдемі күлкі және дұрыс тістеу. Біз оңтайлы емдеу нұсқасын таңдаймыз: классикалық брекеттерден көрінбейтін элайнерлерге дейін. Әрбір пациент үшін жеке жоспар жасаймыз.',
'Регулярная профессиональная чистка — основа здоровья зубов и дёсен. Мы удаляем зубной камень и налёт, полируем эмаль и проводим реминерализацию. Рекомендуем посещать гигиениста каждые 6 месяцев.':'Тұрақты кәсіби тазалау — тіс пен қызыл иектің денсаулығының негізі. Біз тіс тасы мен қабатын алып тастаймыз, эмальды жылтыратамыз және реминерализация жүргіземіз. Әр 6 айда гигиенистке баруды ұсынамыз.',
...
'Современная стоматология для детей и взрослых в Алматы. Безболезненное лечение, профессиональная команда и индивидуальный подход к каждому пациенту.':'Алматыдағы балалар мен ересектерге арналған заманауи стоматология. Ауырсызданусыз емдеу, кәсіби команда және әрбір пациентке жеке тәсіл.',
'Мы находимся в удобном месте в Алматы. Свяжитесь с нами любым удобным способом или приходите лично.':'Біз Алматыда ыңғайлы жерде орналасқанбыз. Бізбен кез келген ыңғайлы тәсілмен байланысыңыз немесе өзіңіз келіңіз.',
};
...
function translatePage(toLang){
  ...
  function walk(node){
    ...
    if(node.nodeType===3){const t=node.textContent.trim(); if(t&&d[t]){node.textContent=d[t];} return;}
    if(node.nodeType===1){
